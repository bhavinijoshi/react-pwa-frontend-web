...
class ImagesUploader extends Component {
  ...
  render() {
    const { max, readonly, classes, disabled } = this.props;
    ...
    const uploadButton = (
      ...
      >
        {loading && <CircularProgress className={classes.loader} color="primary" size={20} />}
      ...
      <div>
        ...
        {this.value() && this.value().length ? (
          ...
          >
            {this.value().map((item) => {
              return (
                <div className={classes.card} key={item.id || item._id}>
                  <img
                    alt={item.name}
                    src={item.publicUrl}
