import actions from 'modules/asset/list/assetListActions';
...
export default (state = initialData, { type, payload }) => {
  ...
  if (type === actions.TOGGLE_VIEW) {
    return {
      ...state,
      isShowView: !state.isShowView,
      selectedRowToView: payload
    }
  }
  ...
  if (type === actions.BLEGATEWAY_FETCH_STARTED) {
    return {
      ...state,
      loading: true,
      selectedKeys: [],
      bleGatewayFilter: payload ? payload.filter : {},
      pagination:
        payload && payload.keepPagination
          ? state.pagination
          : {
  ...
  if (type === actions.ARCFLASH_FETCH_STARTED) {
    return {
      ...state,
      loading: true,
      selectedKeys: [],
      arcFlashFilter: payload ? payload.filter : {},
      pagination:
        payload && payload.keepPagination
          ? state.pagination
          : {
